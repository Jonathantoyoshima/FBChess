...
function snapshot(roomID) {
  ...
  firebase.firestore().collection('room').doc(roomID).collection('registry')
  ...
  var query = room.where("openRoom", "==", true).limit(1).get().then(function(snap){
    if(snap.empty){
      ...
      snap.forEach(function(doc){
        ...
        room.doc(doc.id).update({ whoPlay:whoPlay, player2:playerID, openRoom:false }).then(function(req){
          registrer_moviment();
        });
      });
    };
